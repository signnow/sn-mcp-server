...
class CreateDocumentGroupTemplateRequest(BaseModel):
    ...
    folder_id: Optional[str] = Field(None, description="ID of the templates folder")
...
class CreateDocumentGroupTemplateFromGroupRequest(BaseModel):
    ...
    folder_id: Optional[str] = Field(None, description="Folder where to save the template (must be Templates folder or subfolder)")


class AddTemplateToDocumentGroupTemplateRequest(BaseModel):
    """Request model for adding template to document group template."""
    document_id: Optional[str] = Field(None, description="ID of the document to add")
    template_id: Optional[str] = Field(None, description="ID of the template to add")
...
class CreateDocumentGroupFromTemplateRequest(BaseModel):
    ...
    group_name: str = Field(..., description="Name of the document group")
    client_timestamp: Optional[str] = Field(None, description="Timestamp of client")
    folder_id: Optional[str] = Field(None, description="ID of the folder")
...
class DocumentGroupTemplateRecipientReminder(BaseModel):
    ...
    remind_after: Optional[int] = Field(None, description="Send reminder after specified days")
    remind_before: Optional[int] = Field(None, description="Send reminder before expiration")
    remind_repeat: Optional[int] = Field(None, description="Send reminder every specified days")
...
class DocumentGroupTemplateRecipientAuthentication(BaseModel):
    ...
    value: Optional[str] = Field(None, description="Password value for password authentication")
    phone: Optional[str] = Field(None, description="Phone number for phone authentication")
    method: Optional[str] = Field(None, description="Method for phone: 'sms' or 'phone_call'")
...
class DocumentGroupTemplateRecipientAttributes(BaseModel):
    ...
    message: Optional[str] = Field(None, description="Invite email message")
    subject: Optional[str] = Field(None, description="Invite email subject")
    expiration_days: Optional[int] = Field(None, description="Days until invite expires (3-180)")
    reminder: Optional[DocumentGroupTemplateRecipientReminder] = Field(None, description="Reminder email settings")
    allow_forwarding: Optional[bool] = Field(None, description="Allow recipients to reassign invite")
    i_am_recipient: Optional[bool] = Field(None, description="Document sender is also recipient")
    show_decline_button: Optional[bool] = Field(None, description="Show decline button on signature fields")
    redirect_uri: Optional[str] = Field(None, description="URL after recipient completes document")
...
class DocumentGroupTemplateRecipient(BaseModel):
    ...
    attributes: Optional[DocumentGroupTemplateRecipientAttributes] = Field(None, description="Recipient attributes")
...
class EditDocumentGroupTemplateRecipientsRequest(BaseModel):
    ...
    unmapped_documents: Optional[List[UnmappedDocument]] = Field(None, description="List of unmapped documents")
    allowed_unmapped_sign_documents: Optional[List[AllowedUnmappedSignDocument]] = Field(None, description="List of allowed unmapped sign documents")
    cc: Optional[List[str]] = Field(None, description="List of cc recipient emails (max 100)")
...
class CreateDocumentGroupEmbeddedEditorRequest(BaseModel):
    ...
    redirect_uri: Optional[str] = Field(None, description="Link that opens after editing the document group")
...
class CreateDocumentGroupEmbeddedSendingRequest(BaseModel):
    ...
    redirect_uri: Optional[str] = Field(None, description="Page that opens after embedded sending has been set up")
    redirect_target: Optional[str] = Field("self", description="Redirect target: 'blank' (new tab) or 'self' (same tab)")
    link_expiration: Optional[int] = Field(15, description="Link expiration in minutes (15-45, max: 43200 for Admin users)")
    type: Optional[str] = Field("manage", description="Sending step: 'manage' (Add documents), 'edit' (editor), 'send-invite' (Send Invite page)")
...
class DocumentGroupV2EmailGroup(BaseModel):
    ...
    id: Optional[str] = Field(None, description="Email group ID")
    name: Optional[str] = Field(None, description="Email group name")
...
class DocumentGroupV2FieldInvite(BaseModel):
    ...
    email_group: Optional[DocumentGroupV2EmailGroup] = Field(None, description="Email group if used")
...
class DocumentGroupV2Document(BaseModel):
    ...
    folder_id: Optional[str] = Field(None, description="Folder ID where document is stored")
    owner: DocumentGroupV2DocumentOwner = Field(..., description="Document owner information")
    thumbnail: DocumentGroupV2Thumbnail = Field(..., description="Document thumbnail URLs")
    origin_document_id: Optional[str] = Field(None, description="Original document ID if this is a copy")
    has_unassigned_field: bool = Field(..., description="Whether document has unassigned fields")
    has_credit_card_number: bool = Field(..., description="Whether document contains credit card numbers")
    field_invites: List[DocumentGroupV2FieldInvite] = Field(..., description="List of field invites for this document")
    shared_with_team: Optional[str] = Field(None, description="Team sharing information")


class DocumentGroupV2FreeformInvite(BaseModel):
    """Freeform invite information in v2 response."""
    id: Optional[str] = Field(None, description="Freeform invite ID")
    last_id: Optional[str] = Field(None, description="Last freeform invite ID")
...
class DocumentGroupV2Data(BaseModel):
    ...
    invite_id: Optional[str] = Field(None, description="Current invite ID")
    pending_step_id: Optional[str] = Field(None, description="ID of the pending step")
    state: str = Field(..., description="Current state of the document group (e.g., 'pending')")
    last_invite_id: Optional[str] = Field(None, description="ID of the last invite")
    owner_email: str = Field(..., description="Email of the document group owner")
    folder_id: Optional[str] = Field(None, description="Folder ID where group is stored")
    documents: List[DocumentGroupV2Document] = Field(..., description="List of documents in the group")
    owner: DocumentGroupV2Owner = Field(..., description="Owner information")
    cc_emails: List[str] = Field(..., description="List of CC email addresses")
    freeform_invite: DocumentGroupV2FreeformInvite = Field(..., description="Freeform invite information")

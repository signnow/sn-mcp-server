...
class Settings(BaseSettings):
    ...
    allowed_redirects: str = Field(
        default="http://localhost,http://127.0.0.1",
        description="Comma-separated list of allowed redirect URIs",
        alias="ALLOWED_REDIRECTS"
    )
    
    # OAuth RSA key configuration
    oauth_rsa_private_pem: str | None = Field(
        default=None,
        description="OAuth RSA private key in PEM format",
        alias="OAUTH_RSA_PRIVATE_PEM"
    )
    oauth_jwk_kid: str = Field(
        default="mcp-dev-key",
        description="OAuth JWK key ID",
        alias="OAUTH_JWK_KID"
    )
        ...
        if self.oauth_rsa_private_pem:
            try:
                return serialization.load_pem_private_key(
                    self.oauth_rsa_private_pem.encode(), 
                    password=None
                )
            except ValueError:
                pass

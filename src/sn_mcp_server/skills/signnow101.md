...
When the user wants to upload a file as a **reusable template** (a blueprint they will clone into documents), call `upload_template` instead of `upload_document`:

- Sources: an `@`-attached file (`resource_uri`), a local file path (`file_path`), or a public URL (`file_url`) — the same three sources as `upload_document`.
- After upload succeeds, present the returned `next_steps` and let the user choose:

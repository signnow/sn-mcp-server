...
from signnow_client.models.templates_and_documents import DocumentResponse, GetDocumentFieldsResponse
from signnow_client.models.document_groups import GetDocumentGroupV2Response
from .models import UploadDocumentResponse, DocumentGroupDocument, DocumentField, DocumentGroup, UpdateDocumentFields, FieldToUpdate, UpdateDocumentFieldsResponse, UpdateDocumentFieldsResult
...
def _get_full_document(
    ...
    for field in document_response.fields:
        if field.type == "text":
            document_fields.append(DocumentField(
                id=field.id,
                type=field.type,
...
def _get_full_document_group(
    client: SignNowAPIClient,
    token: str,
    group_data: GetDocumentGroupV2Response
) -> DocumentGroup:
    """
...
def _get_document(
    ...
    token: str,
    entity_id: str,
    entity_type: Optional[Literal["document", "document_group"]] = None
) -> DocumentGroup:
    """
...
def _get_single_document_as_group(
    client: SignNowAPIClient,
    token: str,
    document_id: str,
    document_data: DocumentResponse
...
def _update_document_fields(
    client: SignNowAPIClient,
    token: str,
    update_requests: List[UpdateDocumentFields]
    ...
    for update_request in update_requests:
        try:
            ...
            from signnow_client.models.templates_and_documents import PrefillTextFieldsRequest, PrefillTextField
            prefill_request = PrefillTextFieldsRequest(
                fields=[PrefillTextField(field_name=field.name, prefilled_text=field.value) for field in update_request.fields]
            )
            
            # Update fields using the client
            success = client.prefill_text_fields(
                token=token,
                document_id=update_request.document_id,
                request_data=prefill_request
            )
            
            results.append(UpdateDocumentFieldsResult(
                document_id=update_request.document_id,
                updated=success,
                reason=None  # No reason needed for success
            ))

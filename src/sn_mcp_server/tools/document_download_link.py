...
from signnow_client import SignNowAPIClient, MergeDocumentsRequest
from .models import DocumentDownloadLinkResponse


def _get_document_download_link(
    entity_id: str, 
    entity_type: Optional[Literal["document", "document_group"]],
    token: str,
    ...
    if entity_type == "document_group":
        ...
        if len(document_ids) == 1:
            return client.get_document_download_link(token, document_ids[0])
        
        # Merge all documents in the group
        merge_request = MergeDocumentsRequest(
            name=document_group.group_name,
        ...
        return client.get_document_download_link(token, merge_response.document_id)
    else:
        # For single document, just get its download link
        return client.get_document_download_link(token, entity_id)

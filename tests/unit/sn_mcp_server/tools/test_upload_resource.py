...
`_resolve_upload_resource` backs the `resource_uri` branch of both the
`upload_document` and `upload_template` tools: it reads an attached MCP
resource via `ctx.read_resource`, validates it carries binary content, and
infers a filename from the URI when the caller didn't supply one.
"""

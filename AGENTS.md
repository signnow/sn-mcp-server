...
- **Tool versioning: contract changes and new tools go through the latest MCP version only.** Every `mcp.tool` registration carries an explicit `version=` (enforced by `tests/unit/sn_mcp_server/test_tool_versions.py`). Released contracts are **frozen**: never change a v1.0 or v2.0 tool's input/output parameters in place. Any contract change (adding/removing/renaming an argument or response field) or brand-new tool is introduced as a **v3.0** registration in `tools/signnow_v3.py`, leaving the prior version registered and unchanged. FastMCP serves the highest registered version by default, so clients transparently get the newest contract while a client pinning an older version keeps the old one. Module layout: `tools/signnow_v1.py` (frozen v1.0 compat), `tools/signnow.py` (frozen v2.0), `tools/signnow_v3.py` (v3.0 — where new contract changes land). Business logic in `tools/<feature>.py` is version-agnostic and shared across versions. When a new tool has no released predecessor, register it directly at the current latest version (v3.0) rather than v2.0.
- **Signing reminders use the native resend endpoints.** `send_invite_reminder` resends pending invites the same way the SignNow web app's "Send reminder" action does: documents via `PUT /fieldinvite/{field_invite_id}/resend` (one call per pending field invite), document groups via `POST /documentgroup/{id}/groupinvite/{invite_id}/resendinvites` (one call per pending signer). These reuse the invite's original email template, so the tool's `subject`/`message` parameters are accepted for input-contract stability but not applied. SignNow may reject a resend with `400 "could not resend"` (invite not in a resendable state) — surfaced per-recipient in `failed` — or rate-limit it with `429 "Too Many Attempts"`; because the loops resend once per pending recipient, a 429 is retried (`_resend_with_retry`) before being recorded as failed. The retry honors the server's `Retry-After` header when it is short (≤ `_RESEND_MAX_RETRY_AFTER_SECONDS`); a longer `Retry-After` fails fast (recorded as failed, retryable by the agent) rather than blocking the tool call, and when no `Retry-After` is sent it falls back to exponential backoff. `Retry-After` is parsed once in the client (`_parse_retry_after`) onto `SignNowAPIError.retry_after`. While waiting out a backoff the wait is split into chunks of at most `_RESEND_KEEPALIVE_INTERVAL_SECONDS` (`_backoff_sleep`), each followed by a keepalive `ctx.report_progress` (`_report_resend_wait`) — clients often abort a call that goes silent for more than ~2s, so a multi-second wait (e.g. an honored Retry-After) must keep emitting progress rather than block silently. (Both endpoints are undocumented but live-verified; the older `email2` "send document copy" client method remains available but is no longer used for reminders.)
